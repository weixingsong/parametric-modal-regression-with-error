...
#############MCCL_2##############
 ...
 for(j in seq(n)) {Z[j,]=X1[j,]+U[nj*(j-1)+1,]}
 Zmean=Z
 Temp1=eigen(covu)
 Temp2=(Temp1$vectors)%*%diag(sqrt(Temp1$values))%*%t((Temp1$vectors))
 Zcov=c(Temp2)
 ...
 par(mfrow=c(2,2))
 boxplot.matrix(Tresult[,1:4],xlab="Boxplots of Beta")
 abline(h=-0.25)
 boxplot(Tresult[,5],xlab="Boxplot of phi")
 abline(h=5)
 
 boxplot.matrix(TBcov[,1:4],xlab="Boxplots of std of Beta")
 boxplot(TBcov[,5],xlab="Boxplot of std of phi")
 ...
 TBtnumsum=rbind(TBtM,TBtMD,TBtS,TBtIQ)
 dimnames(TBtnumsum)=list(c("Mean","Median",
                         "Stdev","IQR"),
                       c("beta0","beta1","beta2","beta3","phi"))

 # Numerical summary of beta and phi
 
 TSdM=apply(TBcov,2,mean)
 TSdMD=apply(TBcov,2,median)
 TSdS=apply(TBcov,2,sd)
 TSdIQ=apply(TBcov,2,IQR)
 
 TSdnumsum=rbind(TSdM,TSdMD,TSdS,TSdIQ)
 dimnames(TSdnumsum)=list(c("Mean","Median",
                         "Stdev","IQR"),
                       c("beta0","beta1","beta2","beta3","phi"))
...
resultNM=cbind(result[,1],Tresult[,1],Nresult[,1],result[,2],Tresult[,2],Nresult[,2],result[,3],Tresult[,3],Nresult[,3],result[,4],Tresult[,4],Nresult[,4])
boxplot.matrix(resultNM,xlab="Boxplots of Beta",main="X1 and X2 are dependent")
abline(h=-0.25)

te=Sys.time()
time=te-ts
save(file="Table3laplace.RData",list=ls())
