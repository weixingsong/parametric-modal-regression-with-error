  ...
  while(mm<M)
   {
    ...
    tildZm=rep(0,n)     ### container of mean of tiaotaZ's at each hatX1
    tildZsd=rep(0,n) ### container of covariance of tiaotaZ's at each hatX1
    
    for(i in 1:n)
     {
      Si=Zcov[i]
      tildu=rnorm(nj,0,Si)         ##produce measurement error tiaotau
    ...
    ZmBt=rep(tildZm,each=B) # repeat each row of tiaotaZm B times  
    ZvBt=rep(tildZsd,each =B) # repeat each row of tiaotaZcov B times 
...
save(file="1.y<5000.mccl.RData",list=ls())
