...
abline(h=-0.25,lty=3)
title(main = expression(bold(X)[1]~'a'*'n'*'d'~bold(X)[2]~'a'*'r'*'e'~'i'*'n'*'d'*'e'*'p'*'e'*'n'*'d'*'e'*'n'*'t'))

#resultNM=cbind(result[,1],Nresult[,1],result[,2],Nresult[,2],result[,3],Nresult[,3],result[,4],Nresult[,4])
#boxplot.matrix(resultNM,xlab="Boxplots of Beta",main="X1 and X2 are independent")
#abline(h=-0.25)

te=Sys.time()
time=te-ts
save(file="figure1.独立伯努利.RData",list=ls())
